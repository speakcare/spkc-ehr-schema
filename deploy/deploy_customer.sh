...
ROLE_TRUST_LAMBDA_POLICY_FILE="role_trust_lambda_policy.json"
...
api_gateway_create_route53_record() {
  recordName=$1
  zoneId=$(route53_get_zone_id $SPEAKCARE_DOMAIN $SPEAKCARE_HOSTED_ZONE)
  read regionalDomainName regionalHostedZoneId < <(api_gateway_get_domain_info $recordName)
  echo "Creating Route 53 record for $recordName"
  route53_change_resource_record_sets $recordName $zoneId $regionalHostedZoneId $regionalDomainName
}
...
speakcare_deploy_customer() {
  ...
  # ─── CREATE IAM ROLE ───────────────────────────────────────────────────────
  iam_create_role $ROLE_NAME_LAMBDA_RECORDING $ROLE_TRUST_LAMBDA_POLICY_FILE
  ...
  # Add a Route 53 alias record for the custom domain
  api_gateway_create_route53_record $CUSTOMER_API_DOMAIN
